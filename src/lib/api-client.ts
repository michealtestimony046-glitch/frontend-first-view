...
export interface RunChapter { title: string; startTimestamp: number; }
export interface RunScreenshot { filename: string; timestamp: number; t?: number; label: string; url?: string; }
export interface RunError { subtype: string; timestamp: number; message: string; status?: number; target?: string; x?: number; y?: number; }
export interface RunSummary { assertionsPassed: number; assertionsFailed: number; hardErrorCount: number; bugCount: number; }
