    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          <button onClick={() => { router.invalidate(); reset(); }} className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:opacity-90">Retry</button>
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "apple-mobile-web-app-title", content: "Matrix QA" },
    ],
    links: [
...
function RootComponent() {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <AuthProvider>
        <div key={location.href} className="route-transition">
          <Outlet />
        </div>
      </AuthProvider>
    </QueryClientProvider>
