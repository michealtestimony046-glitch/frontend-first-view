...
export function V2RunPreflight({ project, initialTargetUrl, onClose, onStarted }: Props) {
  ...
  const estimate = plan?.estimatedUnits ?? billing?.estimatedUnits ?? null;
  const readyToStart = Boolean(plan && plan.status !== "FAILED" && plan.scenarios.length > 0 && blockedPolicies.length === 0);
  ...
  const prepare = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setPhase("idle");
      setError(cause instanceof Error ? cause.message : "Unable to prepare a fresh V2 test run.");
    }
  };
  ...
  const start = async () => {
    ...
    try {
      ...
      const response = await v2Api.runPlan(approved.id, { targetUrl: normalizeTargetUrl(targetUrl) || undefined, accessMode, enableVision, enableRecovery });
      ...
      setPhase("ready");
      setError(cause instanceof Error ? cause.message : "Unable to reserve Matrix Units and start the V2 run.");
    }
  };
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-background/70 p-4 backdrop-blur-sm sm:items-center">
    <div className="surface-card w-full max-w-xl overflow-hidden">
      ...
      <form onSubmit={(event) => { void prepare(event); }} className="space-y-4 p-5">
        ...
        {phase === "ready" && plan && <div className="space-y-3 rounded-md border border-primary/30 bg-primary/5 p-4"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-sm font-semibold text-primary"><CheckCircle2 className="h-4 w-4" /> Fresh AI-authored plan ready</div><p className="mt-1 text-xs text-muted-foreground">{plan.scenarios.length} AI-authored scenarios · {plan.mode.replaceAll("_", " ")}</p><p className="mt-2 max-w-md text-xs leading-5 text-foreground/80">Mission: {aiPlan?.missionSummary ?? plan.projectMap?.mission?.goal ?? missionGoal}</p>{plan.projectMap?.coverageFrontier?.counts && <p className="mt-1 text-xs text-muted-foreground">Coverage frontier: {plan.projectMap.coverageFrontier.counts.total} discovered items · {plan.projectMap.coverageFrontier.counts.untested} untested · {plan.projectMap.coverageFrontier.counts.blocked} blocked</p>}</div><div className="text-right"><div className="font-display text-2xl font-semibold">{estimate ?? "—"} ⟐</div><div className="text-[10px] uppercase tracking-wider text-muted-foreground">estimated Matrix Units</div></div></div><div className="rounded-md border border-primary/20 bg-background/30 p-3 text-xs"><div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-foreground">Planning brain</strong><span className="font-mono text-[10px] text-primary">{plan.plannerProvider ?? aiPlan?.provider ?? "AI"} / {plan.plannerModel ?? aiPlan?.model ?? "configured model"}</span></div><p className="mt-1 leading-5 text-muted-foreground">{plan.plannerRationale ?? aiPlan?.planningRationale ?? "The model selected these scenarios from the fresh discovery evidence."}</p>{aiPlan?.uncoveredAreas?.length ? <p className="mt-2 leading-5 text-warning">Needs review later: {aiPlan.uncoveredAreas.join(" · ")}</p> : null}</div><div className="grid gap-1 border-t border-primary/20 pt-3 text-xs text-muted-foreground"><span>Scenario execution: {billing?.baseScenarioUnits ?? "—"} ⟐</span><span>AI discovery: {billing?.aiDiscoveryUnits ?? 0} ⟐</span><span>AI planning: {billing?.aiPlanningUnits ?? 0} ⟐</span><span className="pt-1 text-[11px]">The estimate is shown before browser execution. Unused reserved units are refunded during settlement.</span></div>{blockedPolicies.length > 0 && <div className="flex items-start gap-2 rounded-md border border-warning/40 bg-warning/10 p-3 text-xs text-warning"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0" /><span>{blockedPolicies.length} policy decision{blockedPolicies.length === 1 ? "" : "s"} require review before this plan can run. Open the advanced Discovery review to resolve them.</span></div>}</div>}
        {error && <div className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive">{error}</div>}
        <footer className="flex flex-wrap justify-end gap-2 border-t border-border pt-4"><button type="button" onClick={onClose} disabled={busy} className="rounded-md border border-border px-3 py-2 text-sm hover:bg-accent disabled:opacity-40">Cancel</button>{phase === "idle" || phase === "ready" ? <button type={phase === "idle" ? "submit" : "button"} onClick={phase === "ready" ? () => { if (waitingForProvider && queuedResponse) onStarted(queuedResponse); else void start(); } : undefined} disabled={phase === "idle" ? busy || !targetUrl.trim() && !environmentId : busy || (!waitingForProvider && !readyToStart)} className="inline-flex items-center gap-1.5 rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-40">{phase === "idle" ? <><Radar className="h-4 w-4" /> Prepare estimate</> : waitingForProvider ? <><Play className="h-4 w-4" /> Open queued run</> : <><Play className="h-4 w-4" /> Start and reserve {estimate ?? "—"} ⟐</>}</button> : <span className="inline-flex items-center gap-2 rounded-md bg-primary/15 px-4 py-2 text-sm text-primary"><Loader2 className="h-4 w-4 animate-spin" />{phase === "starting" ? "Reserving and starting…" : "Preparing…"}</span>}</footer>
      </form>
    </div>
  </div>;
}
