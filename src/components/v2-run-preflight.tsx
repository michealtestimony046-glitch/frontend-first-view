...
export function V2RunPreflight({ project, initialTargetUrl, initialMissionGoal, autoStart = false, initialTargetAuthorizationConfirmed = false, onClose, onStarted }: Props) {
  ...
  const readyToStart = Boolean(quickScanResult?.status === "COMPLETED" && plan && plan.status !== "FAILED" && plan.scenarios.length > 0 && blockedPolicies.length === 0 && targetAuthorizationConfirmed);
  ...
  const prepare = async (event?: React.FormEvent) => {
    ...
    try {
      ...
      const quickScanTargetUrl = normalizeTargetUrl(selectedEnvironment?.baseUrl || normalizedTargetUrl);
      const completedQuickScan = await quickScanApi.run({ targetUrl: quickScanTargetUrl, ownershipConfirmed: targetAuthorizationConfirmed });
      setQuickScanResult(completedQuickScan);
      if (completedQuickScan.status !== "COMPLETED") throw new Error(completedQuickScan.errorMessage || "Quick Scan could not reach this target.");
  ...
  const start = async (planOverride?: V2TestPlan) => {
    ...
    const quickScanHandoff = toQuickScanHandoff(quickScanResult);
    if (!quickScanHandoff) {
      setError("Complete the Quick Scan before starting the browser test.");
      setPhase("ready");
      return;
    }
    setError(null);
    ...
    try {
      ...
      const response = await v2Api.runPlan(approved.id, { targetUrl: runTargetUrl || undefined, accessMode, enableVision, enableRecovery, targetAuthorizationConfirmed, quickScanHandoff });
  ...
  return <div role="dialog" aria-modal="true" aria-labelledby="matrixqa-preflight-title" className="fixed inset-0 z-50 flex items-end justify-center bg-background/70 p-4 backdrop-blur-sm sm:items-center">
    <div className="surface-card max-h-[calc(100vh-2rem)] w-full max-w-xl overflow-y-auto sm:max-h-[calc(100vh-3rem)]">
      <header className="flex items-start justify-between gap-4 border-b border-border p-5">
        <div className="flex items-start gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-md bg-primary/15 text-primary"><Radar className="h-5 w-5" /></span><div><h2 id="matrixqa-preflight-title" className="font-display text-lg font-semibold">Prepare a browser test</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">Every run starts with a fast Quick Scan, then creates a fresh adaptive plan before browser execution. Quick Scan findings guide the agent as hypotheses, never as proof.</p></div></div>
        ...
        {phase === "ready" && plan && (
          <div className="space-y-3 rounded-md border border-primary/30 bg-primary/5 p-4">
            ...
            <div className="rounded-md border border-primary/20 bg-background/30 p-3 text-xs">
              <div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-foreground">Quick Scan handoff ready</strong><span className="font-mono text-[10px] uppercase tracking-wider text-primary">hypotheses only</span></div>
              <p className="mt-1 leading-5 text-muted-foreground">{quickScanResult?.findings.length ?? 0} structural lead{quickScanResult?.findings.length === 1 ? "" : "s"} will be available to the browser agent. It may verify, investigate, leave untested, or discover different issues from live evidence.</p>
            </div>
            <div className="rounded-md border border-primary/20 bg-background/30 p-3 text-xs">
