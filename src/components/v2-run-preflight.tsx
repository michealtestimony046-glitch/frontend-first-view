...
export function V2RunPreflight({ project, initialTargetUrl, onClose, onStarted }: Props) {
  ...
  const start = async () => {
    ...
    try {
      ...
      setPhase("ready");
      setError(cause instanceof Error ? cause.message : "Unable to reserve Matrix Units and start the browser test. You can review the estimate and try again when capacity is available.");
    }
  };
