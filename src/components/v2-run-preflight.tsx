...
export function V2RunPreflight({ project, initialTargetUrl, initialMissionGoal, autoStart = false, initialTargetAuthorizationConfirmed = false, onClose, onStarted }: Props) {
  ...
  const [networkProfile, setNetworkProfile] = useState<NetworkProfileType>("FAST");
  ...
  const start = async (planOverride?: V2TestPlan) => {
    ...
    try {
      ...
      const response = await v2Api.runPlan(approved.id, { targetUrl: runTargetUrl || undefined, accessMode, enableVision, enableRecovery, targetAuthorizationConfirmed, networkProfile });
  ...
  return <div role="dialog" aria-modal="true" aria-labelledby="matrixqa-preflight-title" className="fixed inset-0 z-50 flex items-end justify-center bg-background/70 p-4 backdrop-blur-sm sm:items-center">
    <div className="surface-card max-h-[calc(100vh-2rem)] w-full max-w-xl overflow-y-auto sm:max-h-[calc(100vh-3rem)]">
      ...
      <form onSubmit={(event) => { void prepare(event); }} className="space-y-4 p-5">
        ...
        <label className="flex items-start gap-2 rounded-md border border-primary/20 bg-primary/5 p-3 text-xs"><input type="checkbox" checked={targetAuthorizationConfirmed} onChange={(event) => setTargetAuthorizationConfirmed(event.target.checked)} disabled={busy} className="mt-0.5 accent-primary" /><span><span className="flex items-center gap-1.5 font-medium text-foreground"><ShieldCheck className="h-3.5 w-3.5 text-primary" />I own or am authorized to test this target</span><span className="mt-0.5 block leading-5 text-muted-foreground">The Step 0 scan and browser agent will use this target only for the authorized QA run.</span></span></label>
        ...
        {phase === "idle" && <p className="rounded-md border border-border/60 bg-surface-2/30 p-3 text-xs leading-5 text-muted-foreground">Create a fresh map and run-specific plan first. The browser worker will execute Step 0 Quick Scan before functional actions. The browser starts only after the plan is ready.</p>}
        {phase === "ready" && plan && <div className="rounded-md border border-border/70 bg-surface-2/30 p-3 text-xs"><div className="font-medium text-foreground">Network condition</div><div className="mt-2 grid gap-2 sm:grid-cols-3">{NETWORK_PROFILE_TYPES.map((profile) => { const display = NETWORK_PROFILE_DISPLAY[profile]; return <label key={profile} title={display.tooltip} className={`flex cursor-pointer items-start gap-2 rounded-md border p-2.5 ${networkProfile === profile ? "border-primary bg-primary/10" : "border-border"}`}><input type="radio" name="network-profile" value={profile} checked={networkProfile === profile} onChange={() => setNetworkProfile(profile)} disabled={busy} className="mt-0.5 accent-primary" aria-label={display.accessibilityText} /><span><span className="font-medium">{display.label}</span><span className="mt-0.5 block text-[11px] leading-4 text-muted-foreground">{display.tooltip}</span></span></label>; })}</div></div>}
        ...
        {phase === "ready" && plan && <div className="grid gap-2 rounded-md border border-border/70 bg-surface-2/30 p-3 text-xs"><div className="font-medium text-foreground">Optional run capabilities</div><label className="flex cursor-pointer items-start gap-2"><input type="checkbox" checked={enableVision} onChange={(event) => setEnableVision(event.target.checked)} disabled={busy} className="mt-0.5 accent-primary" /><span><span className="flex items-center gap-1.5 font-medium"><Eye className="h-3.5 w-3.5 text-primary" />Enable vision checks</span><span className="mt-0.5 block text-muted-foreground">Off by default. Lets the worker request visual screenshot analysis when text evidence is insufficient.</span></span></label><label className="flex cursor-pointer items-start gap-2"><input type="checkbox" checked={enableRecovery} onChange={(event) => setEnableRecovery(event.target.checked)} disabled={busy} className="mt-0.5 accent-primary" /><span><span className="flex items-center gap-1.5 font-medium"><RefreshCw className="h-3.5 w-3.5 text-primary" />Enable recovery attempts</span><span className="mt-0.5 block text-muted-foreground">Off by default. Lets the worker try approved equivalent locators when the primary locator is not actionable.</span></span></label><p className="text-[11px] text-muted-foreground">Capacity handling is managed automatically, and any provider wait is retried without requiring another submission.</p></div>}
        {(phase === "scanning" || phase === "planning") && <div aria-live="polite" className="rounded-md border border-primary/30 bg-primary/5 p-4"><div className="flex items-center gap-2 text-sm font-medium text-primary"><Loader2 className="h-4 w-4 animate-spin" />{phase === "scanning" ? "Mapping the current application…" : "Building the run-specific plan…"}</div><p className="mt-2 text-xs leading-5 text-muted-foreground">This can take a little longer on targets with many links or when the AI provider is waking up. Keep this window open; the request has a two-minute safety budget.</p><div className="mt-3 grid gap-2 text-xs text-muted-foreground"><span>01 · Step 0 Quick Scan runs in the backend browser worker</span><span className={scan?.status === "COMPLETED" ? "text-success" : ""}>02 · Fresh read-only discovery {scan?.status === "COMPLETED" ? "complete" : "waiting"}</span>{phase === "scanning" && scan?.status !== "COMPLETED" && <><span className="text-foreground">Current step: {scanProgressLabel}</span>{typeof scanProgress?.pagesScanned === "number" && <span>{scanProgress.pagesScanned}/{scanProgress.maxPages ?? 8} pages captured · {scanProgress.queuedUrls ?? 0} queued</span>}{scanProgress?.lastRoute && <span className="truncate">Last route: {scanProgress.lastRoute}</span>}</>}<span className={plan ? "text-success" : ""}>03 · Test planning {plan ? "complete" : "waiting"}</span><span>04 · Policy review waiting</span></div></div>}
        {phase === "ready" && plan && (
          <div className="space-y-3 rounded-md border border-primary/30 bg-primary/5 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2 text-sm font-semibold text-primary"><CheckCircle2 className="h-4 w-4" /> Fresh test plan ready</div>
                <p className="mt-1 text-xs text-muted-foreground">{plan.scenarios.length} test scenarios · {plan.mode.replaceAll("_", " ")}</p>
                {selectedViewports.length > 0 && (
                  <div className="mt-2 space-y-1">
                    <p className="text-xs text-foreground/80">{selectedViewports.length} isolated viewport{selectedViewports.length === 1 ? "" : "s"} · {selectedViewports.map((viewport) => `${viewport.label} (${viewport.width}×${viewport.height})`).join(" · ")}</p>
                    <p className="text-[11px] text-muted-foreground">The same approved scenarios will run once in each viewport context. Estimates scale with the selected viewport count.</p>
                  </div>
                )}
              </div>
              <div className="text-right"><div className="font-display text-lg font-semibold text-success">Ready</div><div className="text-[10px] uppercase tracking-wider text-muted-foreground">fresh plan prepared</div></div>
            </div>
            <p className="max-w-md text-xs leading-5 text-foreground/80">Mission: {aiPlan?.missionSummary ?? plan.projectMap?.mission?.goal ?? missionGoal}</p>
            {plan.projectMap?.coverageFrontier?.counts && <p className="text-xs text-muted-foreground">Coverage frontier: {plan.projectMap.coverageFrontier.counts.total} discovered items · {plan.projectMap.coverageFrontier.counts.untested} untested · {plan.projectMap.coverageFrontier.counts.blocked} blocked</p>}
            <div className="rounded-md border border-primary/20 bg-background/30 p-3 text-xs">
              <div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-foreground">Quick Scan runs with the test</strong><span className="font-mono text-[10px] uppercase tracking-wider text-primary">Step 0</span></div>
              <p className="mt-1 leading-5 text-muted-foreground">The same Playwright page will run the deterministic structural scan before functional actions. Results will appear in the run Overview.</p>
            </div>
            <div className="rounded-md border border-primary/20 bg-background/30 p-3 text-xs">
