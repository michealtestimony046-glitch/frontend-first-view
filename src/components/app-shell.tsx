...
import {
  ...
  Search, Cog, LogOut, KeyRound, UserCircle, Loader2, PlusCircle, CircleDollarSign, ShieldCheck, Users,
} from "lucide-react";
  ...
  return <div ref={ref} className="relative">
    ...
    {open && <div className="absolute left-0 right-0 top-full z-40 mt-2 overflow-hidden rounded-md border border-border bg-popover shadow-2xl">
      ...
      <div className="border-t border-border p-1">
        {!showCreate ? ownsOrganization ? <p className="px-2 py-2 text-[11px] leading-5 text-muted-foreground">The private alpha currently supports one organization per account. Use the existing organization for now.</p> : <button onClick={() => { setShowCreate(true); setError(null); }} className="flex w-full items-center gap-2 rounded px-2 py-2 text-left text-xs text-primary hover:bg-accent"><PlusCircle className="h-3.5 w-3.5" />Create new organization</button> : <form onSubmit={createWorkspace} className="p-2">
