      ...
      {snapshot && <>
        ...
        <section className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
          <section className="surface-card overflow-hidden">
            ...
            <div className="max-h-[28rem] overflow-y-auto divide-y divide-border pr-1">{snapshot.coverageClaims.map((claim) => <div key={claim.id} className="px-5 py-3"><div className="flex items-center justify-between gap-3"><span className="font-mono text-xs text-foreground">{claim.coverageKey}</span><span className={`font-mono text-[10px] uppercase ${statusClass(claim.status)}`}>{claim.status}</span></div><div className="mt-1 text-[11px] text-muted-foreground">{claim.dimension} · {claim.agentKey} · claimed {formatDate(claim.claimedAt)}</div>{claim.evidenceRefs.length > 0 && <div className="mt-1 font-mono text-[10px] text-muted-foreground">evidence: {claim.evidenceRefs.join(", ")}</div>}{claim.conflictReason && <div className="mt-1 text-[11px] text-warning">{claim.conflictReason}</div>}</div>)}{!snapshot.coverageClaims.length && <p className="p-5 text-sm text-muted-foreground">No coverage claims have been recorded.</p>}</div>
          </section>
