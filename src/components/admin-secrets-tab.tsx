  ...
  return <div className="mt-6 grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
    <section className="surface-card p-5">
      ...
      <form onSubmit={(event) => { void save(event); }} className="mt-5 grid gap-3">
        <label className="grid gap-1 text-xs text-muted-foreground">Reference name<input required pattern="[A-Z][A-Z0-9_]{1,127}" value={name} onChange={(event) => setName(event.target.value.toUpperCase())} placeholder="ZAI_API_KEY" className="rounded-md border border-border bg-surface-2/40 px-3 py-2 text-sm text-foreground" /><span className="text-[11px]">Use this exact name in the AI Models configuration.</span></label>
        <label className="grid gap-1 text-xs text-muted-foreground">Secret value<input required type={showValue ? "text" : "password"} value={value} onChange={(event) => setValue(event.target.value)} placeholder="Paste the provider key once" className="rounded-md border border-border bg-surface-2/40 px-3 py-2 text-sm text-foreground" /></label>
        ...
        <label className="grid gap-1 text-xs text-muted-foreground">Description <span className="font-normal">(optional)</span><input value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Z.ai direct API key" maxLength={500} className="rounded-md border border-border bg-surface-2/40 px-3 py-2 text-sm text-foreground" /></label>
